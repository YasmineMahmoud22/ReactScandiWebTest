...
import {CartBtn,NumberOFItems} from "./styles/CartButtonStyle";
  ...
  state ={
    ...
    handleClickOutside(event) {
        if (this.wrapperRef && !this.wrapperRef?.current?.contains(event.target)) {
          this.setState({shwoCart:false,})
        }
    }
    DisplayCart = ()=>{
      this.setState(prevState=>{
        return{
          shwoCart : !prevState.shwoCart,
        }
      })
    }
    
      render(){
        const {shwoCart}=this.state;
        if(shwoCart){
        ...
        return(
          <div>
            <CartBtn onClick={this.DisplayCart}>
                   <i>
                      <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                       ...
                       {NumberofProducts===0
                       ?null
                       :<NumberOFItems>{NumberofProducts}</NumberOFItems>}
            </CartBtn>
           {shwoCart && 
            <MiniCart wrapperRef={this.wrapperRef}  DisplayCart={this.DisplayCart}/>
            }
          </div>
        )
    }}
