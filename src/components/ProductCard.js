...
import {CardContainer,Card, DefaultAddButton, CardImage, CardContent, ProductName, Price, OutOfStock} from "./styles/ProductCardStyle";
import {incrementProduct} from "../actions/cart";


class ProductCard extends React.Component{
    defaultAdd=(product)=>{
        const ChackProductExistance = this.props.cart.some((item)=>
              item.product.name === product.name
        );
        const ProductKeyInCart = this.props.cart.filter((item)=>
             item.product.name === product.name
        );
        if (!ChackProductExistance){
            this.props.addProduct({
                product:product,
                attributesValue:Object.assign({},...(product.attributes.map((attribute)=>
                { return {
            ...
            this.props.increment(ProductKeyInCart[0].key,1)
    ...
    render(){
        ...
        return(
                <CardContainer>
                      <Card nohover={product.inStock?  'nohover':''}>
                      <Link to={`/products/${product.id}`}>
                       <CardImage   backgroundImage = {`url(${product.gallery[0]})`} ></CardImage>
                        <CardContent>
                         <ProductName>{`${product.name}`}</ProductName>
                         <Price>{`${price[0].currency.symbol}`}{`${price[0].amount}`}</Price>
                        </CardContent>
                      </Link>                          
                        <DefaultAddButton onClick={()=>{this.defaultAdd(product)}}>
...
const mapDispatchToProps =(dispatch)=>({
  sendCurrency :(currency)=> dispatch(sendCurrency(currency)),
